...
void ofxOpenVR::exit() {
  ...
  for (std::vector<CGLRenderModel *>::iterator i = _vecRenderModels.begin();
       i != _vecRenderModels.end(); i++) {
    delete (*i);
  }
...
void ofxOpenVR::update() {
  ...
  if (_iTrackedControllerCount != _iTrackedControllerCount_Last ||
      _iValidPoseCount != _iValidPoseCount_Last) {
    ...
    // _strPoseClasses.c_str(), _iTrackedControllerCount);
    printf("PoseCount:%d Controllers:%d\n", _iValidPoseCount,
           _iTrackedControllerCount);
  }
...
void ofxOpenVR::render() {
  ...
  if (_pHMD) {
    ...
    vr::Texture_t leftEyeTexture = {
        (void *)(uintptr_t)leftEyeDesc._nResolveTextureId,
        vr::TextureType_OpenGL, vr::ColorSpace_Gamma};
    vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture);
    vr::Texture_t rightEyeTexture = {
        (void *)(uintptr_t)rightEyeDesc._nResolveTextureId,
        vr::TextureType_OpenGL, vr::ColorSpace_Gamma};
...
glm::mat4x4 ofxOpenVR::getHMDMatrixProjectionEye(vr::Hmd_Eye nEye) {
  ...
  vr::HmdMatrix44_t mat =
      _pHMD->GetProjectionMatrix(nEye, _fNearClip, _fFarClip);
...
//--------------------------------------------------------------
glm::mat4x4 ofxOpenVR::getControllerPose(
    vr::ETrackedControllerRole nController) {
  glm::mat4x4 matrix;
...
//--------------------------------------------------------------
void ofxOpenVR::setClearColor(ofFloatColor color) { _clearColor.set(color); }

//--------------------------------------------------------------
void ofxOpenVR::showMirrorWindow() { vr::VRCompositor()->ShowMirrorWindow(); }

//--------------------------------------------------------------
void ofxOpenVR::hideMirrorWindow() { vr::VRCompositor()->HideMirrorWindow(); }
...
bool ofxOpenVR::init() {
  ...
  _pRenderModels = (vr::IVRRenderModels *)vr::VR_GetGenericInterface(
  ...
  _strTrackingSystemName =
      getTrackedDeviceString(_pHMD, vr::k_unTrackedDeviceIndex_Hmd,
                             vr::Prop_TrackingSystemName_String);
  _strTrackingSystemModelNumber = getTrackedDeviceString(
      _pHMD, vr::k_unTrackedDeviceIndex_Hmd, vr::Prop_ModelNumber_String);
...
bool ofxOpenVR::createAllShaders() {
  ...
  vertex += STRINGIFY(
      uniform mat4 matrix; layout(location = 0) in vec4 position;
      layout(location = 1) in vec3 v3ColorIn; out vec4 v4Color; void main() {
  ...
  _controllersTransformShader.setupShaderFromSource(GL_FRAGMENT_SHADER,
                                                    fragment);
  ...
  vertex = "#version 410\n";
  vertex += STRINGIFY(layout(location = 0) in vec4 position;
                      layout(location = 1) in vec2 v2UVredIn;
                      layout(location = 2) in vec2 v2UVGreenIn;
                      layout(location = 3) in vec2 v2UVblueIn;
                      noperspective out vec2 v2UVred;
                      noperspective out vec2 v2UVgreen;
                      noperspective out vec2 v2UVblue; void main() {
                        v2UVred = v2UVredIn;
  ...
  fragment +=
      STRINGIFY(uniform sampler2D mytexture;

                noperspective in vec2 v2UVred; noperspective in vec2 v2UVgreen;
                ...
                void main() {
                  float fBoundsCheck =
                      ((dot(vec2(lessThan(v2UVgreen.xy, vec2(0.05, 0.05))),
                            vec2(1.0, 1.0)) +
                        dot(vec2(greaterThan(v2UVgreen.xy, vec2(0.95, 0.95))),
                            vec2(1.0, 1.0))));
                  if (fBoundsCheck > 1.0) {
  ...
  vertex = "#version 410\n";
  vertex +=
      STRINGIFY(uniform mat4 matrix; layout(location = 0) in vec4 position;
                layout(location = 1) in vec3 v3NormalIn;
                layout(location = 2) in vec2 v2TexCoordsIn; out vec2 v2TexCoord;
                void main() {
  ...
  fragment = "#version 410\n";
  fragment += STRINGIFY(
      uniform sampler2D diffuse; in vec2 v2TexCoord; out vec4 outputColor;
      void main() { outputColor = texture(diffuse, v2TexCoord); });
...
//--------------------------------------------------------------
bool ofxOpenVR::createFrameBuffer(int nWidth, int nHeight,
                                  FramebufferDesc &framebufferDesc) {
  // Still using direct OpenGL calls to create the FBO as OF does not allow the
  // create of GL_TEXTURE_2D_MULTISAMPLE texture.
  ...
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                            GL_RENDERBUFFER, framebufferDesc._nDepthBufferId);
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                         ...
                         framebufferDesc._nRenderTextureId, 0);
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         framebufferDesc._nResolveTextureId, 0);
...
void ofxOpenVR::updateDevicesMatrixPose() {
  ...
  vr::VRCompositor()->WaitGetPoses(_rTrackedDevicePose,
  ...
  for (int nDevice = 0; nDevice < vr::k_unMaxTrackedDeviceCount; ++nDevice) {
    if (_rTrackedDevicePose[nDevice].bPoseIsValid) {
      ...
      _rmat4DevicePose[nDevice] = convertSteamVRMatrixToMatrix4(
          _rTrackedDevicePose[nDevice].mDeviceToAbsoluteTracking);

      // Add info to the debug panel.
      switch (_pHMD->GetTrackedDeviceClass(nDevice)) {
        case vr::TrackedDeviceClass_Controller:
          if (_pHMD->GetControllerRoleForTrackedDeviceIndex(nDevice) ==
              vr::TrackedControllerRole_LeftHand) {
            _strPoseClassesOSS << "Controller Left" << endl;
          } else if (_pHMD->GetControllerRoleForTrackedDeviceIndex(nDevice) ==
        ...
        case vr::TrackedDeviceClass_GenericTracker:
          _strPoseClassesOSS << "Generic trackers, similar to controllers"
                             << endl;
          break;
      ...
      if (_pHMD->GetTrackedDeviceClass(nDevice) ==
  ...
  if (_rTrackedDevicePose[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid) {
    _mat4HMDPose =
        glm::inverse(_rmat4DevicePose[vr::k_unTrackedDeviceIndex_Hmd]);
  }

  // Aad more info to the debug panel.
  _strPoseClassesOSS << endl;
  _strPoseClassesOSS << "Pose Count: " << _iValidPoseCount << endl;
  _strPoseClassesOSS << "Controller Count: " << _iTrackedControllerCount
                     << endl;
}
...
void ofxOpenVR::processVREvent(const vr::VREvent_t &event) {
  ...
  switch (_pHMD->GetTrackedDeviceClass(event.trackedDeviceIndex)) {
    case vr::TrackedDeviceClass_Controller:
      ...
      // Controller's role.
      if (_pHMD->GetControllerRoleForTrackedDeviceIndex(
              event.trackedDeviceIndex) == vr::TrackedControllerRole_LeftHand) {
        _args.controllerRole = ControllerRole::Left;
      } else if (_pHMD->GetControllerRoleForTrackedDeviceIndex(
      ...
      vr::VRSystem()->GetControllerState(event.trackedDeviceIndex,
                                         &pControllerState,
                                         sizeof(pControllerState));
...
void ofxOpenVR::renderStereoTargets() {
  ...
  glBlitFramebuffer(0, 0, _nRenderWidth, _nRenderHeight, 0, 0, _nRenderWidth,
                    _nRenderHeight, GL_COLOR_BUFFER_BIT, GL_LINEAR);
...
void ofxOpenVR::drawControllers() {
  ...
  //   return;
  // } FIXME: Whenever a new API comes out this function may be done differently
  // but for now just hack it together lol
...
void ofxOpenVR::renderScene(vr::Hmd_Eye nEye) {
  ...
  if (_bDrawControllers) {
    ...
    _controllersTransformShader.setUniformMatrix4f(
        "matrix", getCurrentViewProjectionMatrix(nEye), 1);
  ...
  if (_bRenderModelForTrackedDevices) {
    ...
    for (uint32_t unTrackedDevice = 0;
      ...
      if (!_rTrackedDeviceToRenderModel[unTrackedDevice]) continue;

      const vr::TrackedDevicePose_t &pose =
          _rTrackedDevicePose[unTrackedDevice];
      if (!pose.bPoseIsValid) {
        continue;
      }

      // FIXME: Whenever a new API comes out this function may be done
      // differently but for now just hack it together lol
      if (_pHMD->GetTrackedDeviceClass(unTrackedDevice) ==
          vr::TrackedDeviceClass_Controller) {
        continue;
      }

      glm::mat4x4 matMVP = getCurrentViewProjectionMatrix(nEye) *
                           _rmat4DevicePose[unTrackedDevice];
...
void ofxOpenVR::renderDistortion() {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  GL_LINEAR_MIPMAP_LINEAR);
  glDrawElements(GL_TRIANGLES, _uiIndexSize / 2, GL_UNSIGNED_SHORT,
                 (const void *)(_uiIndexSize));
...
void ofxOpenVR::drawDebugInfo(float x, float y) {
  ...
  ofDrawBitmapStringHighlight(_strPoseClassesOSS.str(), ofPoint(x, y),
                              ofColor(ofColor::black, 100.0f));
}

//--------------------------------------------------------------
glm::mat4x4 ofxOpenVR::convertSteamVRMatrixToMatrix4(
    const vr::HmdMatrix34_t &matPose) {
  glm::mat4x4 matrixObj(matPose.m[0][0], matPose.m[1][0], matPose.m[2][0], 0.0,
                        matPose.m[0][1], matPose.m[1][1], matPose.m[2][1], 0.0,
                        matPose.m[0][2], matPose.m[1][2], matPose.m[2][2], 0.0,
                        matPose.m[0][3], matPose.m[1][3], matPose.m[2][3],
                        1.0f);
  return matrixObj;
}
...
//-----------------------------------------------------------------------------
CGLRenderModel *ofxOpenVR::findOrLoadRenderModel(
...
//-----------------------------------------------------------------------------
void ofxOpenVR::setupRenderModelForTrackedDevice(
  ...
  std::string sRenderModelName = getTrackedDeviceString(
      _pHMD, unTrackedDeviceIndex, vr::Prop_RenderModelName_String);
  ...
  if (!pRenderModel) {
    std::string sTrackingSystemName = getTrackedDeviceString(
        _pHMD, unTrackedDeviceIndex, vr::Prop_TrackingSystemName_String);
